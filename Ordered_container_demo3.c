...
int main(void)
{
	...
	struct Point s3 = {2, 1};
	struct Point s4 = {2, 2};
	struct Point s1clone = { 1, 1 };
	struct Point s1clone2 = { 1, 1 };
	struct Point s2clone = {1, 2};
	struct Point s3clone = {2, 1};
	struct Point s4clone = {2, 2};
	...
	printf("\ninserting (%d, %d)\n", s1clone.x, s1clone.y);
	OC_insert(container, &s1clone);
	printf("size is %d\n", OC_get_size(container));
	OC_apply(container, print_as_Point);

	printf("\ninserting (%d, %d)\n", s2clone.x, s2clone.y);
	OC_insert(container, &s2clone);
	printf("size is %d\n", OC_get_size(container));
	OC_apply(container, print_as_Point);

	printf("\ninserting (%d, %d)\n", s3clone.x, s3clone.y);
	OC_insert(container, &s3clone);
	printf("size is %d\n", OC_get_size(container));
	OC_apply(container, print_as_Point);

	printf("\ninserting (%d, %d)\n", s1clone2.x, s1clone2.y);
	OC_insert(container, &s1clone2);
	printf("size is %d\n", OC_get_size(container));
	OC_apply(container, print_as_Point);

	printf("\ninserting (%d, %d)\n", s4clone.x, s4clone.y);
	OC_insert(container, &s4clone);
	printf("size is %d\n", OC_get_size(container));
	OC_apply(container, print_as_Point);

	find_and_remove(container, s1);
	find_and_remove(container, s1);
	find_and_remove(container, s1);
	find_and_remove(container, s1);

	find_and_remove(container, s2);
	find_and_remove(container, s2);

	find_and_remove(container, s3);
	find_and_remove(container, s3);

	find_and_remove(container, s4);
	find_and_remove(container, s4);
